const styles = {
    ...
    starting: 'background: #FFE135; color: #000000; font-weight: bold; padding: 4px 8px; border-radius: 4px;',
...
console.log(`${logPrefix}Create by. Wanzaty`, styles.info);
console.log(`${logPrefix}Join Grup : t.me/takeflightnode`, styles.starting);
...
function clickSequentially(elements) {
    if (isGamePaused) return;

    const clickNext = index => {
        if (index < elements.length) {
            clickElement(elements[index]);
            setTimeout(() => clickNext(index + 1), randomDelay(100, 300));
        }
    };
    ...
    if (!isGamePaused) {
        try {
            ...
            if (modal) closeModal();
                ...
                if (cards.length >= 2) {
                    clickSequentially([cards[0], cards[1]]);
                ...
                if (targetArea) {
                    ...
                    handleEndGame();
                    clickAfterClose(); // Ensure we handle any close modals after game actions
                }
            }
        } catch (error) {
